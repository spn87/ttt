...
class plgContentBesps extends JPlugin
{
	//Constructor
	function plgContentBesps( &$subject )
	{
		parent::__construct( $subject );
		// load plugin parameters
		$this->_plugin = JPluginHelper::getPlugin( 'content', 'besps' );
	...
	function onPrepareContent(&$row, &$params, $limitstart) {
		...
		// checking
		if ( !preg_match("#{besps}(.*?){/besps}#s", $row->text) ) {
			return;
		}
		...
		$path_ctrls 	= '/images/stories/besps_buttons/'; // button folder
		$path_plugin 	= '/plugins/content/plugin_besps/'; // path to plugin folder
		$file_css 		= 'besps.css'; 											// default stylesheet
		$file_js 			= 'besps.js'; 											// default JavaScript

		// import helper
    JLoader::import( 'bespshelper', dirname( __FILE__ ).'/plugin_besps' );
		
		$document =& JFactory::getDocument();

//captions
		if (preg_match_all("#{besps_c}(.*?){/besps_c}#s", $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
			foreach ($matches[0] as $match) {
				$_raw_cap_ = preg_replace("/{.+?}/", "", $match);
				$_raw_cap_exp_ = explode("|",$_raw_cap_);
				$cap1=($_raw_cap_exp_[1]&&trim($_raw_cap_exp_[1])!="")?(trim(plgContentBespsHelper::beStrtolower($_raw_cap_exp_[1]))):("CAPDEFAULT");
				...
				$cap4 = plgContentBespsHelper::beKickQuotes($cap2);
				$cap5 = plgContentBespsHelper::beKickQuotes($cap3);
				${$caparray}[$cap1]=array($cap2,$cap3,$cap4,$cap5);
				//remove the call
				$row->text = plgContentBespsHelper::beReplaceCall("{besps_c}".$_raw_cap_."{/besps_c}",'', $row->text);
			}
		}
//captions

//links
		if (preg_match_all("#{besps_l}(.*?){/besps_l}#s", $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
			foreach ($matches[0] as $match) {
				$_raw_link_ = preg_replace("/{.+?}/", "", $match);
				$_raw_link_exp_ = explode("|",$_raw_link_);
				$_link1=($_raw_link_exp_[1]&&trim($_raw_link_exp_[1])!="")?(plgContentBespsHelper::beStrtolower(trim($_raw_link_exp_[1]))):("LINKDEFAULT");
				...
				//remove the call
				$row->text = plgContentBespsHelper::beReplaceCall("{besps_l}".$_raw_link_."{/besps_l}",'', $row->text);
			}
		}
//links

//images
		if (preg_match_all("#{besps}(.*?){/besps}#s", $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
			$bs_count = -1;
			foreach ($matches[0] as $match) {
				$bs_count++;
				...
				if(count($besps_raw)>=2){ //there are parameteroverrides
					for($i=1;$i<count($besps_raw);$i++){
						...
						if(count($overr_temp)>=2){
							$besps_overrides[plgContentBespsHelper::beStrtolower(trim($overr_temp[0]))]=trim($overr_temp[1]);
						}
					}
				}
				unset($images);
				...
				if ($dh = @opendir($path_absolute.$path_imgroot.$_images_dir_)) {
					while (($f = readdir($dh)) !== false) {
						if((substr(plgContentBespsHelper::beStrtolower($f),-3) == 'jpg') || (substr(plgContentBespsHelper::beStrtolower($f),-3) == 'gif') || (substr(plgContentBespsHelper::beStrtolower($f),-3) == 'png')) {
							$noimage++;
				...
				if($noimage) {
					...
					$bs_width_				= (array_key_exists("width",$besps_overrides)&&$besps_overrides['width']!="")?($besps_overrides['width']):($this->_params->get('im_width', 400));
					$bs_height_				= (array_key_exists("height",$besps_overrides)&&$besps_overrides['height']!="")?($besps_overrides['height']):($this->_params->get('im_height', 300));
					$bs_im_align_ 		= (array_key_exists("align",$besps_overrides)&&$besps_overrides['align']!="")?($besps_overrides['align']):($this->_params->get('im_align', 1));
					$bs_bgcol_				= (array_key_exists("bgcol",$besps_overrides)&&$besps_overrides['bgcol']!="")?($besps_overrides['bgcol']):($this->_params->get('sl_bgcol', 'FFFFFF'));
					$bs_sdur_					= (array_key_exists("sdur",$besps_overrides)&&$besps_overrides['sdur']!="")?($besps_overrides['sdur']):($this->_params->get('sl_sdur', 3));
					$bs_fdur_					= (array_key_exists("fdur",$besps_overrides)&&$besps_overrides['fdur']!="")?($besps_overrides['fdur']):($this->_params->get('sl_fdur', 1));
					$bs_steps_				= (array_key_exists("steps",$besps_overrides)&&$besps_overrides['steps']!="")?($besps_overrides['steps']):($this->_params->get('sl_steps', 20));
					$bs_autostart_		= (array_key_exists("auto",$besps_overrides)&&$besps_overrides['auto']!="")?($besps_overrides['auto']):($this->_params->get('autostart', 1));
					$bs_sort_					= (array_key_exists("sort",$besps_overrides)&&$besps_overrides['sort']!="")?($besps_overrides['sort']):($this->_params->get('bs_sort', 0));
					$bs_setid_ 				= (array_key_exists("setid",$besps_overrides)&&$besps_overrides['setid']!="")?($besps_overrides['setid']):($this->_params->get('setid', 0));
					//controls
					$path_ctrls 			= trim($this->_params->get('buttonpath', $path_ctrls));
					$bs_ctrl_show_ 		= (array_key_exists("ctrls",$besps_overrides)&&$besps_overrides['ctrls']!="")?($besps_overrides['ctrls']):($this->_params->get('ctrl_show', 0));
					$bs_ctrl_sort_ 		= (array_key_exists("csort",$besps_overrides)&&$besps_overrides['csort']!="")?(trim($besps_overrides['csort'])):(trim($this->_params->get('ctrl_sort', '1-2345')));
					$bs_ctrl_start_ 	= (array_key_exists("cstart",$besps_overrides))?($besps_overrides['cstart']):(trim($this->_params->get('ctrl_start', '')));
					$bs_ctrl_stop_ 		= (array_key_exists("cstop",$besps_overrides))?($besps_overrides['cstop']):(trim($this->_params->get('ctrl_stop', '')));
					$bs_ctrl_fwd_ 		= (array_key_exists("cfwd",$besps_overrides))?($besps_overrides['cfwd']):(trim($this->_params->get('ctrl_fwd', '')));
					$bs_ctrl_back_ 		= (array_key_exists("cbwd",$besps_overrides))?($besps_overrides['cbwd']):(trim($this->_params->get('ctrl_back', '')));
					$bs_ctrl_height_ 	= 22;
					//###preload
					$prld_decision 		= (array_key_exists("prld",$besps_overrides)&&$besps_overrides['prld']!="")?($besps_overrides['prld']):(intval($this->_params->get('preld', '')));
					$prld_decision=($prld_decision&&$prld_decision>=3)?($prld_decision):("");
					$prld_replace=$path_site.$path_plugin."besps.png";
			    //###captions and links
					$bs_cap_show_ 		= (array_key_exists("caps",$besps_overrides)&&$besps_overrides['caps']!="")?($besps_overrides['caps']):($this->_params->get('cap_show', 1));
					$bs_cap_pos_ 			= (array_key_exists("inout",$besps_overrides)&&$besps_overrides['inout']!="")?($besps_overrides['inout']):($this->_params->get('cap_pos', 1));
					$bs_link_use_ 		= (array_key_exists("links",$besps_overrides)&&$besps_overrides['links']!="")?($besps_overrides['links']):($this->_params->get('link_use', 1));
			    //###files
			    if($this->_params->get('stylesheet')!=='-1'&&$this->_params->get('stylesheet')!=='0'){$file_css=$this->_params->get('stylesheet');}
			    if($this->_params->get('javascript')!=='-1'&&$this->_params->get('javascript')!=='0'){$file_js=$this->_params->get('javascript');}
					
					//allow default buttons
					$bs_ctrl_start_type = $bs_ctrl_stop_type = $bs_ctrl_fwd_type = $bs_ctrl_back_type = "noimg";
					if(is_file($path_absolute.$path_ctrls.$bs_ctrl_start_)&&((substr(plgContentBespsHelper::beStrtolower($bs_ctrl_start_),-3)=='jpg')||(substr(plgContentBespsHelper::beStrtolower($bs_ctrl_start_),-3)=='gif')||(substr(plgContentBespsHelper::beStrtolower($bs_ctrl_start_),-3)=='png'))){$bs_ctrl_start_=$path_ctrls.$bs_ctrl_start_;$bs_ctrl_start_type="img";}
					elseif($bs_ctrl_start_==""){$bs_ctrl_start_=$path_plugin.'start.png';$bs_ctrl_start_type="img";}
					if(is_file($path_absolute.$path_ctrls.$bs_ctrl_stop_)&&((substr(plgContentBespsHelper::beStrtolower($bs_ctrl_stop_),-3)=='jpg')||(substr(plgContentBespsHelper::beStrtolower($bs_ctrl_stop_),-3)=='gif')||(substr(plgContentBespsHelper::beStrtolower($bs_ctrl_stop_),-3)=='png'))){$bs_ctrl_stop_=$path_ctrls.$bs_ctrl_stop_;$bs_ctrl_stop_type="img";}
					elseif($bs_ctrl_stop_==""){$bs_ctrl_stop_=$path_plugin.'stop.png';$bs_ctrl_stop_type="img";}
					if(is_file($path_absolute.$path_ctrls.$bs_ctrl_fwd_)&&((substr(plgContentBespsHelper::beStrtolower($bs_ctrl_fwd_),-3)=='jpg')||(substr(plgContentBespsHelper::beStrtolower($bs_ctrl_fwd_),-3)=='gif')||(substr(plgContentBespsHelper::beStrtolower($bs_ctrl_fwd_),-3)=='png'))){$bs_ctrl_fwd_=$path_ctrls.$bs_ctrl_fwd_;$bs_ctrl_fwd_type="img";}
					elseif($bs_ctrl_fwd_==""){$bs_ctrl_fwd_=$path_plugin.'fwd.png';$bs_ctrl_fwd_type="img";}
					if(is_file($path_absolute.$path_ctrls.$bs_ctrl_back_)&&((substr(plgContentBespsHelper::beStrtolower($bs_ctrl_back_),-3)=='jpg')||(substr(plgContentBespsHelper::beStrtolower($bs_ctrl_back_),-3)=='gif')||(substr(plgContentBespsHelper::beStrtolower($bs_ctrl_back_),-3)=='png'))){$bs_ctrl_back_=$path_ctrls.$bs_ctrl_back_;$bs_ctrl_back_type="img";}
					...
					//sort images
					$images = plgContentBespsHelper::beSortImages($images,$bs_sort_);
					...
					for($a = 0;$a<$noimage;$a++) {
						if($images[$a]['filename'] != '') {
							...
							if(isset($cap_jsarray)){
								...
								if(array_key_exists(plgContentBespsHelper::beStrtolower($images[$a]['filename']),$$captions)){
									$cap_jsarray.='"'.${$captions}[plgContentBespsHelper::beStrtolower($images[$a]['filename'])][2].'","'.${$captions}[plgContentBespsHelper::beStrtolower($images[$a]['filename'])][3].'"';
									if($a==0){$currentarray=${$captions}[plgContentBespsHelper::beStrtolower($images[$a]['filename'])];}
									}
							...
							if($bs_link_use_&&isset($$bslinks)){
								if(array_key_exists(plgContentBespsHelper::beStrtolower($images[$a]['filename']),$$bslinks)){$currentlink=${$bslinks}[plgContentBespsHelper::beStrtolower($images[$a]['filename'])];}
							...
							elseif(isset($$captions)){
								if(array_key_exists(plgContentBespsHelper::beStrtolower($images[$a]['filename']),$$captions)){$curimtitle=${$captions}[plgContentBespsHelper::beStrtolower($images[$a]['filename'])][2];}
					...
					if($bs_ctrl_show_) {
						...
						for($i=0;$i<strlen($bs_ctrl_sort_);$i++){
						  ...
						  if($k>=1){$html.="&nbsp;";}
						    ...
						    case '2': //start
						    	$k++;
									$html.="<a href='javascript:besps_".$row->id."_".$bs_count.".b_start();'>";
						    ...
						    case '3': //stop
						    	$k++;
									$html.="<a href='javascript:besps_".$row->id."_".$bs_count.".b_stop();'>";
						    ...
						    case '4': //back
						    	$k++;
									$html.="<a href='javascript:besps_".$row->id."_".$bs_count.".b_back();'>";
						    ...
						    case '5': //next
						    	$k++;
									$html.="<a href='javascript:besps_".$row->id."_".$bs_count.".b_forw();'>";
				...
				$row->text = plgContentBespsHelper::beReplaceCall("{besps}".$besps_code."{/besps}",$html, $row->text);
			}
