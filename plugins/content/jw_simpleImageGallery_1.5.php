...
$thb_width 							= (!is_null($params->get('thb_width',null))) ? $params->get('thb_width') : (int) $pluginParams->get('thb_width', 200);
$thb_height 						= (!is_null($params->get('thb_height',null))) ? $params->get('thb_height') : (int) $pluginParams->get('thb_height', 160);
$smartResize 						= $pluginParams->get('smartResize', 1);
$jpg_quality 						= (int) $pluginParams->get('jpg_quality', 80);
$galleryMessages				= (!is_null($params->get('galleryMessages',null))) ? $params->get('galleryMessages') : $pluginParams->get('galleryMessages', 1);
$cache_expire_time	 		= (int) $pluginParams->get('cache_expire_time',120) * 60; // Cache expiration time in minutes
...
// Other assignments
$transparent						= $siteUrl."/plugins/content/".$this->plg_name."/includes/images/transparent.gif";
...
$slimboxFolder = (JPluginHelper::isEnabled('system','mtupgrade')) ? 'slimbox-1.71a' : 'slimbox-1.58';

$headIncludes = '
'.JHTML::_('behavior.mootools').'
<script type="text/javascript" src="'.$siteUrl.'/plugins/content/'.$this->plg_name.'/includes/'.$slimboxFolder.'/js/slimbox.js"></script>
<style type="text/css" media="all">
@import "'.$siteUrl.'/plugins/content/'.$this->plg_name.'/includes/'.$slimboxFolder.'/css/slimbox.css";
@import "'.$pluginCSS.'";
</style>
<!--[if lte IE 6]>
<link rel="stylesheet" href="'.$pluginCSSie6.'" type="text/css" />
<![endif]-->
<!--[if IE 7]>
<link rel="stylesheet" href="'.$pluginCSSie7.'" type="text/css" />
<![endif]-->
';
...
if(preg_match_all($regex, $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
	...
	foreach ($matches[0] as $key => $match) {
		...
		if(!$gallery){
			JError::raiseNotice('',JText::_('JW_SIG_PRG').'</div>');
			return;
		}
