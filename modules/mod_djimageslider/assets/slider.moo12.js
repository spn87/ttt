...
this.DJImageSlider = new Class({
    ...
    initialize: function(settings, options){
		...
		function makeFade(prev_slide){
			...
			slides[prev_slide].get('tween',{property: 'opacity', duration: options.duration}).start(0).chain(function(){
		...
		function sliderLoaded(){
			...
			$('djslider' + settings.id).get('tween',{property: 'opacity', duration: 500}).start(1);
