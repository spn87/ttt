...
function com_install()
{
	...
	if (file_exists(JPATH_SITE.DS."components".DS."com_fadegallery".DS."fadegallery.php"))
		...
		echo '<h1>FadeGallery 1.3.3 installed succesfully</h1>
...
fadestep=20"';
	...
	AddColumnIfNotExist($db->getPrefix().'fadegallery', 'randomize', 'int(1)', 'NOT NULL');
...
END;
	...
	";
		...
		$procedurenotallowed=false;
		
			$db->setQuery("DROP PROCEDURE IF EXISTS addcol;" );
			$db->query();
	
			$db->setQuery( $query );
			if (!$db->query()) $procedurenotallowed=true;
		
			
			if(!$procedurenotallowed)
			{
			...
			if($procedurenotallowed)
			{		
				$query="ALTER TABLE `".$tablename."` ADD COLUMN `".$columnname."` ".$filedtype." ".$options.";";
				$db->query();
